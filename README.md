...
- 実機でテストする場合は、Apple Developer アカウントと適切なプロビジョニングプロファイルが必要です
...
アプリケーションには、一時的なネットワーク問題に対応するための自動リトライ機能が実装されています。

#### リトライインターセプター

**実装場所**: `lib/core/interceptors/retry_interceptor.dart`
...
| エラータイプ | 説明 | リトライ |
|------------|------|---------|
| 接続タイムアウト | `DioExceptionType.connectionTimeout` | ✅ |
| 受信タイムアウト | `DioExceptionType.receiveTimeout` | ✅ |
| 送信タイムアウト | `DioExceptionType.sendTimeout` | ✅ |
| 接続エラー | `DioExceptionType.connectionError` | ✅ |
| HTTP 502/503/504/408 | 一時的なサーバーエラー | ✅ |
| HTTP 401/403 | 認証・権限エラー | ❌ |
| HTTP 400/404 | クライアントエラー | ❌ |

**デフォルト設定**:

- 最大リトライ回数: 3回
- リトライ間隔: 1秒（API）、2秒（画像アップロード）
- バックオフ戦略: エクスポネンシャル（1秒 → 2秒 → 3秒）

**適用箇所**:

1. **APIクライアント** (`lib/repositories/api_client.dart`)
...
1. **画像アップロード** (`lib/services/image_cache_service.dart`)
...
- リトライはPOST/PUTリクエストでも実行されるため、APIの冪等性を確保してください
- 過度なリトライはサーバー負荷を増やす可能性があります
- タイムアウト設定は適切に調整してください
